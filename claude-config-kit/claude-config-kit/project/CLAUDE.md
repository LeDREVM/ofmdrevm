...
## 🔧 Tech Stack
- {{TECH_STACK}}
- Build : {{BUILD_CMD}} · Tests : {{TEST_CMD}} · Deploy : {{DEPLOY_CMD}}

## Commands
- `{{BUILD_CMD}}` — build production
- `{{TEST_CMD}}` — tests
- `{{DEPLOY_CMD}}` — déploiement
- `{{DEV_PREVIEW_CMD}}` — preview prod-fidèle
...
- (ajoute ici les invariants propres à {{PROJECT_NAME}})
